...
Replacing new levels in nominal variables with the most frequent level (step_novel).
Converting all nominal (categorical) variables to binary dummy variables (step_dummy). This is necessary because many machine learning algorithms cannot handle categorical data directly.
Removing variables that have zero variance, i.e., the ones that have the same value in every observation (step_zv).
Normalizing all numeric variables to have mean zero and standard deviation one (step_normalize). This is done to ensure that all features have the same scale.
...
In the naive scenario, where every transaction is assumed to be non-fraudulent, the cost of undetected fraud is about 2,075,089 US$. However, with the use of the model, the cost of fraud decreases substantially. The model is able to correctly identify fraudulent transactions worth about 1,600,460 US$, reducing the cost of false negatives to 474,629.3 US$. Although the model mistakenly flags some legitimate transactions as fraudulent (false positives), the associated cost is much lower (146,807.1 US$).
