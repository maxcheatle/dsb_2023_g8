...
# Visualize the distribution of the new time variables
ggplot(card_fraud, aes(x = hour)) +
  geom_histogram(binwidth = 1) +
  theme_minimal() +
  labs(title = "Distribution of transaction hour")

# Visualize the distribution of the distance
ggplot(card_fraud, aes(x = distance_km)) +
  geom_histogram() +
  theme_minimal() +
  labs(title = "Distribution of transaction distance (km)")
...
```{r}
# Let us try to compact the job column into a more general industry column.

# Create categories
education <- c("Higher education careers adviser", "Academic librarian", "Primary school teacher", "Education administrator", "Associate Professor", "Teacher, adult education", "Education officer, community", "English as a foreign language teacher", "Professor Emeritus")
medical <- c("Physiological scientist", "Medical technical officer", "Nurse, mental health", "Pathologist", "Hospital pharmacist", "Oncologist", "Clinical cytogeneticist", "Occupational therapist")
engineering <- c("Engineer, building services", "Engineer, aeronautical", "Engineer, broadcasting (operations)", "Manufacturing systems engineer", "Engineer, manufacturing", "Engineer, civil (consulting)", "Engineer, materials", "Engineer, structural", "Broadcast engineer", "Engineer, site")
management <- c("Quarry manager", "Company secretary", "Retail manager", "Product manager", "Hotel manager", "Restaurant manager, fast food", "Warehouse manager", "Purchasing manager")
arts <- c("Media planner", "Jewellery designer", "Presenter, broadcasting", "Art gallery manager", "Designer, television/film set", "Artist", "Writer", "Dancer", "Designer, multimedia", "Production assistant, television", "Television camera operator", "Producer, television/film/video")
retail <- c("Merchandiser, retail", "Visual merchandiser", "Buyer, retail")
legal <- c("Chartered legal executive (England and Wales)", "Lawyer", "Solicitor, Scotland", "Legal secretary", "Solicitor")
scientific_research <- c("Research scientist (medical)", "Scientist, research (medical)", "Biochemist, clinical", "Ecologist", "Economist", "Data scientist", "Nature conservation officer", "Scientist, clinical (histocompatibility and immunogenetics)", "Chemist, analytical", "Geneticist, molecular", "Hydrographic surveyor", "Analytical chemist", "Water quality scientist", "Hydrogeologist")
administration <- c("Records manager", "Public librarian", "Administrator, arts", "Civil Service administrator", "Chief of Staff", "Civil Service fast streamer", "Personnel officer", "Information officer")
military <- c("Armed forces logistics/support/administrative officer", "Armed forces technical officer")
agriculture <- c("Conservation officer, historic buildings", "Forest/woodland manager")
construction <- c("Architectural technologist")
hospitality <- c("Catering manager")
finance <- c("Insurance claims handler", "Accountant, chartered certified", "Investment banker, operational", "Accountant, chartered")
journalism <- c("Magazine journalist", "Broadcast journalist", "Copy")
aviation <- c("Air broker", "Ambulance person", "Air cabin crew", "Air traffic controller")
sales <- c("Sales promotion account executive")
maritime <- c("Ship broker")
it_consulting <- c("IT consultant", "Programmer, applications")
environmental <- c("Environmental education officer", "Environmental manager")
public_relations <- c("Public relations officer")
real_estate <- c("Estate manager/land agent")
psychology <- c("Clinical psychologist")
charity <- c("Charity officer")
horticulture <- c("Horticultural consultant")
textiles <- c("Designer, textile", "Textile designer")
pharmaceutical <- c("Pharmacist, hospital")
govt_officer <- c("Local government officer")
community <- c("Community development worker")
sports <- c("Sports administrator")
museum <- c("Museum education officer", "Conservator, museum/gallery", "Conservator, furniture")
telecom <- c("Broadcast engineer")
animal_care <- c("Veterinary surgeon")
alternative_medicine <- c("Homeopath")

# Assign industry based on the job
card_fraud$industry <- NA # initializing a new column with NA
card_fraud$industry[card_fraud$job %in% education] <- 'Education'
card_fraud$industry[card_fraud$job %in% medical] <- 'Medical/Healthcare'
card_fraud$industry[card_fraud$job %in% engineering] <- 'Engineering'
card_fraud$industry[card_fraud$job %in% management] <- 'Management'
card_fraud$industry[card_fraud$job %in% arts] <- 'Arts/Creative'
card_fraud$industry[card_fraud$job %in% retail] <- 'Retail'
card_fraud$industry[card_fraud$job %in% legal] <- 'Legal'
card_fraud$industry[card_fraud$job %in% scientific_research] <- 'Scientific Research'
card_fraud$industry[card_fraud$job %in% administration] <- 'Administration'
card_fraud$industry[card_fraud$job %in% military] <- 'Military'
card_fraud$industry[card_fraud$job %in% agriculture] <- 'Agriculture'
card_fraud$industry[card_fraud$job %in% construction] <- 'Construction'
card_fraud$industry[card_fraud$job %in% hospitality] <- 'Hospitality'
card_fraud$industry[card_fraud$job %in% finance] <- 'Finance'
card_fraud$industry[card_fraud$job %in% journalism] <- 'Journalism'
card_fraud$industry[card_fraud$job %in% aviation] <- 'Aviation'
card_fraud$industry[card_fraud$job %in% sales] <- 'Sales'
card_fraud$industry[card_fraud$job %in% maritime] <- 'Maritime'
card_fraud$industry[card_fraud$job %in% it_consulting] <- 'IT Consulting'
card_fraud$industry[card_fraud$job %in% environmental] <- 'Environmental'
card_fraud$industry[card_fraud$job %in% public_relations] <- 'Public relations officer'
card_fraud$industry[card_fraud$job %in% real_estate] <- 'Real Estate'
card_fraud$industry[card_fraud$job %in% psychology] <- 'Psychology'
card_fraud$industry[card_fraud$job %in% charity] <- 'Charity'
card_fraud$industry[card_fraud$job %in% horticulture] <- 'Horticulture'
card_fraud$industry[card_fraud$job %in% textiles] <- 'Textiles'
card_fraud$industry[card_fraud$job %in% pharmaceutical] <- 'Pharmaceutical'
card_fraud$industry[card_fraud$job %in% govt_officer] <- 'Government Officer'
card_fraud$industry[card_fraud$job %in% community] <- 'Community Development'
card_fraud$industry[card_fraud$job %in% sports] <- 'Sports'
card_fraud$industry[card_fraud$job %in% museum] <- 'Museum'
card_fraud$industry[card_fraud$job %in% telecom] <- 'Telecommunications'
card_fraud$industry[card_fraud$job %in% animal_care] <- 'Animal Care'
card_fraud$industry[card_fraud$job %in% alternative_medicine] <- 'Alternative Medicine'
